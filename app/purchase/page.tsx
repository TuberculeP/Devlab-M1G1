...
export default function PurchasePoints() {
  ...
  const [purchasePoints, setPurchasePoints] = useState<CollectPointsResponse | null>(null);
  ...
  const fetchPurchasePoints = async () => {
    setIsLoading(true);
    setError(null);

    const params = new URLSearchParams();
    params.append('type', 'purchase_point');
    if (selectedCity !== 'all') {
  ...
  const getMapUrl = () => {
    ...
    if (selectedCity !== "all") {
      ...
      if (!city) {
        ...
        return `${baseUrl}search?key=${api_key}&q=point+de+vente+hauts+de+seine&center=${defaultCoords}&zoom=11`;
      }
      return `${baseUrl}search?key=${api_key}&q=point+de+vente+${selectedCity}&center=${city.coordinates}&zoom=14`;
    }
    const defaultCoords = cities.length > 0 ? cities[0].coordinates : "48.8922,2.2417";
    return `${baseUrl}search?key=${api_key}&q=point+de+vente+hauts+de+seine&center=${defaultCoords}&zoom=11`;
  };
  ...
  if (error) {
    return (
      <div className="container mx-auto px-4 pt-24 pb-10">
        <p className="text-red-500">Une erreur est survenue lors du chargement des points de vente: {error}</p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 pt-24 pb-10">
      ...
      {purchasePoints?.total && purchasePoints.total > limit && (
